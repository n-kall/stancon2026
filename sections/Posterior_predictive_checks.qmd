...
The disadvantage of "diagonal line" is preciselly that all the "action" is close to the diagonal and then most of the plot is just blank space. A simple *trick* to improve the *data-ink* ratio is to render the difference between the observed and expected cumulative distribution functions, the $\Delta$-ECDF, as shown in @fig-post_ppc_pit_ecdf. The ideal $\Delta$-ECDF will be a horizontal line at zero and deviations becomes peaks and valleys in the plot. 
...
# Using ArviZ we can visualize the ETI coverage by setting
# ``coverage=True`` in the `plot_ppc_pit` function.
...
# ArviZ offers many LOO-based functions
# `loo_pit` is one of them.
az.plot_loo_pit(dt);
```
## R
```{.R}
