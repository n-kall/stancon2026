...
[Course materials](https://n-kall.github.io/stancon2026-workflow-tools)
...
tt() |>
format_tt(j = "rhat", fn = function(x) sprintf("%.3f", x))
```
...
cens <- readRDS("/home/osvaldo/proyectos/06_Tutoriales/stancon2026-workflow-tools/models/prerun/censored_cats.rds")
ppc_km_overlay(cens$observed_data, cens$posterior_predictive[1:50, ],
               status_y = cens$constant_data)
```

- Predictive curves capped relative to max observed duration (`extrapolation_factor`, default 1.2 → up to 20% beyond max observation)
