...
Once successfully installed, you will need to restart the notebook using the new environment. e.g. `uv run jupyter notebook` or select a different kernel in your editor.
...
```{python}
az.plot_forest(eight_schools_draws, var_names=["theta"], combined=True)
```
...
Next plot sensitivity using density plots. Plot only the mu and tau variables.
...
az.plot_psense_dist(dt_nc, var_names=["mu", "tau"])
...
# Load the draws
draws1 = az.from_netcdf('data/draws1.nc')
draws2 = az.from_netcdf('data/draws2.nc')
draws3 = az.from_netcdf('data/draws3.nc')
draws4 = az.from_netcdf('data/draws4.nc')

# Get the actual observations and the corresponding species
y = draws1.observed_data['count'].values
species = draws1.constant_data['species'].values  # useful for grouped plots

# Create yrep arrays for posterior predictive checks
# (ArviZ convention: posterior_predictive variable matches observed_data name)
yrep1 = draws1.posterior_predictive['count']
yrep2 = draws2.posterior_predictive['count']
yrep3 = draws3.posterior_predictive['count']
yrep4 = draws4.posterior_predictive['count']
